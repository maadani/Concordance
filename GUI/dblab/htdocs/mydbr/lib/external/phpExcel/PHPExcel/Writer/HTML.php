	...
	public function generateSheetData() {
		...
		foreach ($sheets as $sheet) {
			...
			if ($this->_isPdf)
			{
				...
				$sheet_count = $this->_phpExcel->getSheetCount();
				
				// We'll access the next sheet if available
				// Do not put pagebreak in last sheet ($sheetId < $sheet_count-1)
				
				
				if ($sheetId<$sheet_count-1 && $this->_phpExcel->getSheet($sheetId+1)->getPageBreak() ) {
					switch ($this->_phpExcel->getSheet($sheetId+1)->getPageSetup()->getOrientation()) {
						case 'portrait':
							$pagebreak_params = ' params="'.urlencode(serialize(array('P'))).'"';
							break;
						case 'landscape':
							$pagebreak_params = ' params="'.urlencode(serialize(array('L'))).'"';
							// $pagebreak_params = ' params="L"';
							break;
						default:
							$pagebreak_params = '';
							break;
					}
					$html .= '<tcpdf method="AddPage"'.$pagebreak_params.' />'."\n";
				}
