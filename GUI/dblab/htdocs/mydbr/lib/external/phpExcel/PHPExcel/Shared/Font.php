...
class PHPExcel_Shared_Font
{
	...
	{
		global $mydbr_defaults, $MYDBR_PDF_FONT_WIDTH;

		$fontName = $font->getName();
		$fontSize = $font->getSize();
		$font_width = $MYDBR_PDF_FONT_WIDTH;

		// Calculate column width in pixels. We assume fixed glyph width. Result varies with font name and size.
		switch ($fontName) {
			case 'Calibri':
				$pdf_font_width = $font_width ? $font_width : 8.26;
				// value 8.26 was found via interpolation by inspecting real Excel files with Calibri 11 font.
				$columnWidth = (int) ($pdf_font_width * PHPExcel_Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 11; // extrapolate from font size
				break;
			case 'Arial':
				$pdf_font_width = $font_width ? $font_width : 7;
			...
			case 'Verdana':
				$pdf_font_width = $font_width ? $font_width : 8;
