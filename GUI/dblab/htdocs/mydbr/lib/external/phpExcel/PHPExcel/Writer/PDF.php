...
class PHPExcel_Writer_PDF extends PHPExcel_Writer_HTML implements PHPExcel_Writer_IWriter {
	...
	public function save($pFilename = null) {
		...
		// 
		$mydbr_pdf = $mydbr_defaults['export']['pdf'];
		
		$logo = $mydbr_pdf['logo'] ? ''.$mydbr_pdf['logo'] : '';
		$pdf->setHeaderData( $logo, $mydbr_pdf['logo_width'], $this->_phpExcel->getProperties()->getTitle(),
		 	print_cell(date("Y-m-d H:i:s"),'datetime'));
		$pdf->setPrintHeader(true);
		$pdf->setPrintFooter(true);
		$pdf->setImageScale($mydbr_pdf['image_scale']);
...
//		$pdf->setFooterMargin($printMargins->getFooter() * 72);
		...
		$header_footer_font = array('freesans', 'B', 8);
		$pdf->SetTopMargin(50);
		$pdf->SetHeaderMargin(PDF_MARGIN_HEADER);
		$pdf->setHeaderFont( $header_footer_font );
		$pdf->setFooterFont( $header_footer_font );
