...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
	...
	*/
	...
	// For myDBR
	public function setPageBreak($set)
	{
		$this->_pageBreak = $set;
	}
	...
	public function calculateColumnWidths($calculateMergeCells = false)
	{
		...
		if (!empty($autoSizes)) {
			...
			foreach ($this->getCellCollection(false) as $cellID) {
				...
				// myDBR, wrapped columns do not autosize (titles)
				$wrap = $this->getStyle($cellID)->getAlignment()->getWrapText();
				if ($wrap && isset($autoSizes[$cell->getColumn()])) {
